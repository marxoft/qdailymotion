...
#ifndef STRONGTAGSMODEL_H
#define STRONGTAGSMODEL_H
...
class StrongtagsModelPrivate;

class QDAILYMOTIONSHARED_EXPORT StrongtagsModel : public Model
{
    Q_OBJECT
    ...
    Q_PROPERTY(QDailymotion::StrongtagsRequest::Status status READ status NOTIFY statusChanged)
    Q_PROPERTY(QDailymotion::StrongtagsRequest::Error error READ error NOTIFY statusChanged)
    Q_PROPERTY(QString errorString READ errorString NOTIFY statusChanged)
                
public:
    explicit StrongtagsModel(QObject *parent = 0);
    ...
    StrongtagsRequest::Status status() const;
    
    StrongtagsRequest::Error error() const;
...
private:        
    Q_DECLARE_PRIVATE(StrongtagsModel)
    Q_DISABLE_COPY(StrongtagsModel)
